...
const getFeedsSummary = `-- name: GetFeedsSummary :many
SELECT feeds.name, feeds.url, users.name as user_name
    FROM feeds JOIN users ON feeds.user_id = users.id
`

type GetFeedsSummaryRow struct {
	Name     string
	Url      string
	UserName string
}

func (q *Queries) GetFeedsSummary(ctx context.Context) ([]GetFeedsSummaryRow, error) {
	rows, err := q.db.QueryContext(ctx, getFeedsSummary)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetFeedsSummaryRow
	for rows.Next() {
		var i GetFeedsSummaryRow
